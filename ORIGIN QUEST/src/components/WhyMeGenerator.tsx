...
const WhyMeGenerator = () => {
  ...
  const addMotivationEventFromForm = () => {
    if (newEvent.description.trim()) {
      const event = {
        ...
        year: newEvent.year,
        emotionLevel: newEvent.emotionLevel
      };
      setFormData(prev => ({
        ...prev,
        motivationEvents: [...prev.motivationEvents, event]
  ...
  const removeMotivationEvent = (id) => {
    setFormData(prev => ({
      ...prev,
      motivationEvents: prev.motivationEvents.filter(event => event.id !== id)
    }));
  };
                    ...
                    <div className="relative h-64 overflow-x-auto">
                      <div className="flex items-end h-full min-w-full" style={{ width: '800px' }}>
                        {Array.from({ length: 11 }, (_, i) => {
                          ...
                          return (
                            <div key={year} className="flex-1 relative h-full flex flex-col justify-end">
                              {/* イベントプロット */}
        ...
        return (
          <div className="space-y-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 dark:text-gray-300">
                Will・Can・Needから、あなただけのWhy Meを生成します
              </p>
            </div>
            ...
            {!formData.generatedWhyMe && (
              <div className="text-center">
                <button
                  ...
                  className="px-8 py-4 bg-purple-600 text-white rounded-xl hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-lg font-medium"
                >
                  {isGenerating ? (
                    <>
